...
class TestOrders:
    ...
    async def test_create_order_unauthorized(self, client: AsyncClient, test_product):
        ...
        order_data = {"items": [{"product_id": test_product.id, "quantity": 1}]}
    ...
    async def test_create_order_insufficient_stock(self, client: AsyncClient, auth_headers, test_product):
        ...
        order_data = {"items": [{"product_id": test_product.id, "quantity": 100}]}  # больше чем есть (10)
        response = await client.post("/api/v1/orders/", json=order_data, headers=auth_headers)
    ...
    async def test_get_order_by_id(self, client: AsyncClient, auth_headers, test_product):
        ...
        # Создаём заказ
        order_data = {"items": [{"product_id": test_product.id, "quantity": 1}]}
        create_response = await client.post("/api/v1/orders/", json=order_data, headers=auth_headers)
